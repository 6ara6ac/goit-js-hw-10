...
countryInput.addEventListener("input", (0, _lodashDebounceDefault.default)(onInputForm, 300));
function onInputForm(evt) {
    evt.preventDefault();
    let item1 = evt.target.value.trim();
    clearInfo();
    if (item1 === "") return;
    (0, _fetchCountries.fetchCountries)(item1).then((item)=>{
        if (item.length > 10) return (0, _notiflixDefault.default).Notify.info("Too many matches found. Please enter a more specific item.");
        if (item.length <= 10 && item.length >= 2) return countryList(item);
        if (item.length === 1) return getCountry(item);
    }).catch((error)=>{
...
},{"./css/styles.css":"1CY4s","./fetchCountries":"fTQqv","lodash.debounce":"3JP5n","notiflix":"5z0Oc","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"1CY4s":[function() {},{}],"fTQqv":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "fetchCountries", ()=>fetchCountries);
function fetchCountries(name) {
    return fetch(`https://restcountries.com/v2/name/${name}`).then((resp)=>{
